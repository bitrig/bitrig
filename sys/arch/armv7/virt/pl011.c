...
struct cfdriver pl011uart_cd = {
	NULL, "pl011", DV_TTY
};

struct cfattach pl011uart_ca = {
...
{
	struct pl011_softc *sc = pl011uart_cd.cd_devs[DEVUNIT(tp->t_dev)];
	//bus_space_tag_t iot = sc->sc_iot;
	//bus_space_handle_t ioh = sc->sc_ioh;
...
{
	int unit = DEVUNIT(dev);
	struct pl011_softc *sc = pl011uart_cd.cd_devs[unit];
	//bus_space_tag_t iot = sc->sc_iot;
	//bus_space_handle_t ioh = sc->sc_ioh;
	struct tty *tp = sc->sc_tty;
	int s;
