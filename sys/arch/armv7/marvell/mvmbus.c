...
void	 mvmbus_parse_ranges(struct mvmbus_softc *, void *);
...
mvmbus_match(struct device *parent, void *cfdata, void *aux)
{
	...
	/* TODO: support more than the armada 370 / xp */
	if (fdt_node_compatible("marvell,armada370-mbus", aa->aa_node) ||
	    fdt_node_compatible("marvell,armadaxp-mbus", aa->aa_node))
		return (1);
...
mvmbus_attach(struct device *parent, struct device *self, void *args)
{
	...
	mbusc = fdt_find_node_by_phandle_prop(aa->aa_node, "controller");
	if (mbusc == NULL)
		panic("%s: cannot find mbus controller", __func__);

	if (fdt_get_memory_address(mbusc, 0, &mem))
		panic("%s: could not extract memory data from FDT",
		    __func__);

	if (bus_space_map(sc->sc_iot, mem.addr, mem.size, 0, &sc->sc_mbus_ioh))
		panic("%s: bus_space_map failed!", __func__);

	if (fdt_get_memory_address(mbusc, 1, &mem))
		panic("%s: could not extract memory data from FDT",
		    __func__);

	if (bus_space_map(sc->sc_iot, mem.addr, mem.size, 0, &sc->sc_sdram_ioh))
		panic("%s: bus_space_map failed!", __func__);

	if (fdt_get_memory_address(mbusc, 2, &mem))
		panic("%s: could not extract memory data from FDT",
		    __func__);

	if (bus_space_map(sc->sc_iot, mem.addr, mem.size, 0, &sc->sc_bridge_ioh))
		panic("%s: bus_space_map failed!", __func__);
	...
	if (sc->sc_windows == NULL)
		panic("%s: can't allocate window information buffer");

	for (int i = 0; i < sc->sc_num_wins; i++)
		mvmbus_disable_window(sc, i);

	mvmbus_parse_dram_info(sc);
	mvmbus_dram_info = &sc->sc_dram_info;

	mvmbus_parse_ranges(sc, aa->aa_node);
}

void
mvmbus_parse_ranges(struct mvmbus_softc *sc, void *node)
{
	void *parent;
	int pac, psc, cac, csc, ret, rlen, rone, *range;
	...
	/* We only support 32-bit (1), and 64-bit (2) wide addresses here. */
	ret = fdt_node_property_int(node, "#address-cells", &cac);
	if (ret <= 0)
		cac = pac;
	else if (ret > 1 || cac <= 0 || cac > 2)
		return;

	/* We only support 32-bit (1), and 64-bit (2) wide sizes here. */
	ret = fdt_node_property_int(node, "#size-cells", &csc);
	if (ret <= 0)
		csc = psc;
	else if (ret > 1 || csc <= 0 || csc > 2)
		return;
	...
	for (; rlen >= rone; rlen -= rone, range += rone) {
		uint32_t window = betoh32(range[0]);
		if (window & 0xF0000000)
			continue;

		uint32_t size = betoh32(range[cac + pac]);
		if (csc == 2)
			size = betoh32(range[cac + pac + 1]);

		/* All good, extract to address and translate. */
		uint32_t base = betoh32(range[cac]);
		if (pac == 2)
			base = betoh32(range[cac + 1]);
...
mvmbus_disable_window(struct mvmbus_softc *sc, int window)
{
	...
	for (int i = 0; i < sc->sc_num_remappable_wins; i++) {
		bus_space_write_4(sc->sc_iot, sc->sc_mbus_ioh,
